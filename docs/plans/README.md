# TradeX Implementation Plans

These documents record the major TradeX workstreams as separate, as-built
implementation plans. Each plan describes the objective, delivered interfaces,
validation, and extension boundaries for one area of the system.

| Plan | Status | Supporting Guide |
| --- | --- | --- |
| [IBKR Execution](01-ibkr-execution-plan.md) | Implemented | [Secrets and Platforms](../secrets-and-platforms.md) |
| [Kraken Execution](02-kraken-execution-plan.md) | Implemented | [Secrets and Platforms](../secrets-and-platforms.md) |
| [Platform Registry and Secrets](03-platform-registry-and-secrets-plan.md) | Implemented | [Secrets and Platforms](../secrets-and-platforms.md) |
| [S&P 500 Training Pipeline](04-sp500-training-plan.md) | Implemented | [Stock Training](../stock-training.md) |
| [Kraken Crypto Training Pipeline](05-kraken-crypto-training-plan.md) | Implemented | [Crypto Training](../crypto-training.md) |

The supporting guides are operator-focused. The plans in this directory focus
on architecture, implementation scope, acceptance criteria, and future
extension points.
