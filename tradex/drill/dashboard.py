...
async function refresh(){
 ...
 document.getElementById('error').innerHTML='';document.getElementById('session').textContent=`${d.drill.session_date} | ${d.drill.status} | refreshes every 5 seconds`;
 ...
 document.getElementById('signals').innerHTML=d.signals.slice().reverse().map(s=>`<tr><td>${new Date(s.decided_at).toLocaleTimeString()}</td><td>${esc(s.portfolio)}</td><td>${esc(s.symbol)}</td><td>${esc(s.signal)}</td><td>${esc(s.source)}</td><td>${esc(s.reason)}</td></tr>`).join('')||'<tr><td colspan="6">Signals not evaluated yet</td></tr>';
 ...
 document.getElementById('prices').innerHTML=d.prices.slice().reverse().map(p=>`<tr><td>${new Date(p.captured_at).toLocaleTimeString()}</td><td>${esc(p.portfolio)}</td><td>${esc(p.symbol)}</td><td>${money(p.price)}</td><td>${esc(p.source)}</td><td>${new Date(p.source_timestamp).toLocaleString()}</td></tr>`).join('')||'<tr><td colspan="6">No market data captured</td></tr>';
